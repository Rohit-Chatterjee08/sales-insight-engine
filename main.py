...
# NEW AGENT for extracting key findings
key_findings_agent = Agent(
    role="Data Analysis Insights Extractor",
    goal="Extract the top 5-7 most critical, non-obvious insights from an aggregated data summary.",
    backstory="You are a senior data analyst with a keen eye for what matters. Your job is to look at a final data aggregation and extract a short, bulleted list of the most important findings that an executive would want to know.",
    allow_delegation=False, verbose=True, llm=mistral_llm
)
report_writing_agent = Agent(
    role="Senior Business Analyst & Report Writer",
    goal="Take a list of key findings and write a comprehensive, insightful markdown report.",
    backstory="You are a seasoned analyst known for your ability to turn bullet points into a fluid, professional narrative. You write detailed reports for executive review, starting with a summary and elaborating on the provided key findings.",
    allow_delegation=False, verbose=True, llm=mistral_llm
)
...
def run_crew(file_path: str):
    try:
        # --- Stage 1, 2, 3: File Loading, Column Selection, Type Classification ---
        print("Stages 1-3: Loading file, selecting columns, and classifying data types...")
        ...
        # --- Stage 5: Code-based Dashboard Generation & Two-Step AI Reporting ---
        ...
        print("Stage 5b: AI extracting key findings...")
        key_findings_task = Task(
            description=f"Extract the top 5-7 most important, non-obvious insights from this aggregated data. Focus on what an executive would find most interesting. Data:\n\n{final_data_str}",
            expected_output="A bulleted list in markdown format of the key findings.",
            agent=key_findings_agent
        )
        key_findings = _run_resilient_crew(Crew(agents=[key_findings_agent], tasks=[key_findings_task]), "Key Findings Extraction")

        print("Stage 5c: AI writing final report...")
        report_task = Task(
            description=f"Write a detailed business analysis in markdown. Start with an executive summary, then elaborate on the following key findings, and conclude with a brief summary. Key Findings:\n\n{key_findings}",
            expected_output="A comprehensive business report in markdown format.", 
        ...
        if not report_output or len(report_output.split()) < 20:
            report_output = "## Analysis Complete\nThe AI generated the dashboard visuals but did not produce a detailed text summary. This may happen with certain datasets."
